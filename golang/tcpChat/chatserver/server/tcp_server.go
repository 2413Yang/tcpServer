...
var (
	UnknownClient = errors.New("Unknow client")
)
...
func (s *TcpServer) accept(conn net.Conn)*client{
	log.Printf("Accepting connection form %v,total clients:%v",conn.RemotrAddr().String(),len(s.clients)+1)
...
func (s *TcpServer) remove(client *client) {
	...
	log.Printf("Closing connnection from %v",client.conn.RemoteA().String())
	client.conn.Close()
}
